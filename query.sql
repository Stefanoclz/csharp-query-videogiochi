...
SELECT COUNT(id) as NumeroVoti, videogame_id
FROM reviews
GROUP BY videogame_id; 
...
SELECT COUNT(id) as QuantitàVideogiochi, YEAR(release_date)
FROM videogames
...
SELECT player_id
FROM reviews
INNER JOIN players on reviews.player_id = player_id
GROUP BY player_id;
...
INNER JOIN categories on categories.name = category_videogame.category_id;
-- 
-- 4- Selezionare i dati di tutte le software house che hanno rilasciato almeno un gioco dopo il 2020, mostrandoli una sola volta (6)
SELECT software_houses.name, COUNT(videogames.id) as ReleasedAfter2020
FROM software_houses
